...
def query1(tx, req):
    ...
    result = tx.run ("Match (a:account)-[r:made]->(t:trans), (a)-[:BelongTo]->(d:district) where t.date >= date({"
                     ...
                     "d.district_id={dist} return count(t) as count, d.district_name as dist", year=year, month=month, year_larger=year_larger,
...
def query2(tx, req):
    ...
    result = tx.run ("Match (a:account)-[r:made]->(t:trans) where a.account_id={acc} return count(t)", acc=account_id)
    res = result.data ()
    print (res)
    return res
